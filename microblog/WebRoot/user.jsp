  ...
  <body>
    <div id="body">
		...
		<div id="main">
			...
			<div id="center">
				<c:choose>
					<c:when test="${param.fans==null}">
						<p>你关注的用户：</p>
						<c:forEach items="<%=user.getFollows() %>" var="follow">
						<ul>
							<li><a href="userinfo.jsp?userid=${follow.userId}">${follow.userName}</a>&nbsp;<a href="cancelfollow?userid=${follow.userId}">取消关注</a></li>
						</ul>
						</c:forEach>
					</c:when>
					<c:when test="${param.follow==null}">
						<p>关注你的用户：</p>
						<c:forEach items="<%=user.getFans() %>" var="fans">
						<ul>
							<li><a href="userinfo.jsp?userid=${fans.userId}">${fans.userName}</a>&nbsp;<a href="removefans?userid=${fans.userId}">移除粉丝</a></li>
						</ul>
						</c:forEach>
					</c:when>
				</c:choose>
